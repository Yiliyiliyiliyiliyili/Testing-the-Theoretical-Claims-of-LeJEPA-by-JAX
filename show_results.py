...
def print_training_snapshots(name, hist, every=2000):
    print(f"\n--- {name} ---")
    print(f"  {'Step':>6}  {'Loss':>8}  {'SIGReg':>8}  {'Inv Loss':>10}  {'Emb Var':>10}")
    print(f"  {'-'*52}")
...
def plot_baseline(baseline_results):
    ...
    ax.set_title("Baseline: SIGReg Loss")
    ...
    ax.set_title("Baseline: Embedding Variance\ncollapse->0  /  over-regularization>>1")
    ax.legend(); ax.grid(alpha=0.3)
 
    ax = axes[2]
    for name, hist in baseline_results.items():
        ax.plot(hist["steps"], hist["inv"], label=name,
                color=BASELINE_COLORS.get(name, "#555"), linewidth=2.0)
    ax.set_xlabel("Step"); ax.set_ylabel("Invariance Loss")
    ax.set_title("Baseline: Invariance Loss\n(view alignment quality)")
    ax.legend(); ax.grid(alpha=0.3)
...
def plot_main(main_results):
    ...
    # Bottom-left: Invariance loss
    ax = axes[1, 0]
    for name, hist in main_results.items():
        ax.plot(hist["steps"], hist["inv"], label=name,
                color=COLORS.get(name, "#555"), linewidth=lw(name), alpha=0.9)
    ax.set_xlabel("Step"); ax.set_ylabel("Invariance Loss")
    ax.set_title("Invariance Loss  (view alignment quality, lower is better)")
    ax.legend(fontsize=8); ax.grid(alpha=0.3)
 
    # Bottom-right: Gradient ratio
    ax = axes[1, 1]
    for name, hist in main_results.items():
